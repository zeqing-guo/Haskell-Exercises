...
primitiveBindings = nullEnv >>= (flip bindVars $ map (second PrimitiveFunc) primitives ++ map (second IOFunc) ioPrimitives)
...
eval env (List (Atom "define" : DottedList (Atom var : params) varags : body)) = makeVarArgs varags env params body >>= defineVar env var
