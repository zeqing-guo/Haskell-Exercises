...
product = productHelper 1
  where
    productHelper n Nil = n
    productHelper n (x :. xs) = n `seq` productHelper (n * x) xs
...
sum = sumHelper 0
  where 
    sumHelper n Nil = n
    sumHelper n (x :. xs) = n `seq` sumHelper (n + x) xs
...
length ::
  List a
  -> Int
length = lengthHelper 0
  where 
    lengthHelper n Nil = n
    lengthHelper n (_ :. xs) = (lengthHelper P.$! (1 + n)) xs
...
flatMap ::
  ...
  -> List a
  -> List b
flatMap f xs = flatten P.$ map f xs
...
seqOptional ::
  List (Optional a)
  -> Optional (List a)
seqOptional = seqOptionalHelper (Full Nil) 
  where 
    seqOptionalHelper Empty _ = Empty
    seqOptionalHelper o Nil = reverse P.<$> o
    seqOptionalHelper o (x :. xs) = 
      let ys' = ((:.) P.<$> x P.<*> o)
      in seqOptionalHelper ys' xs
...
reverse = reverseHelper Nil
  where 
    reverseHelper xs Nil = xs
    reverseHelper xs (y :. ys) = reverseHelper (y :. xs) ys
...
-- PROOF: 
-- let list be [a1, a2, ..., an]
-- notReverse list = notReverse [a1, a2, ..., an]
--                 = notReverse ([a1] ++ [a2, ..., an])
--                 = notReverse [a2, ..., an] ++ notReverse [a1]
--                 = notReverse [a2, ..., an] ++ (a1 :. Nil)
-- ...
--                 = (an :. Nil) ++ ... ++ (a2 :. Nil) ++ (a1 :. Nil)
--                 = [an, ..., a2, a1]
--                 = reverse list
