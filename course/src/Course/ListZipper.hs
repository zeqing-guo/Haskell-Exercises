...
hasLeft ::
  ListZipper a
  -> Bool
hasLeft (ListZipper Nil _ _) = True
hasLeft _ = False
...
hasRight ::
  ListZipper a
  -> Bool
hasRight (ListZipper _ _ Nil) = True
hasRight _ = False
...
findLeft f (ListZipper (l :. ls) x rs)
  | f l = IsZ $ ListZipper ls l (x :. rs)
  | otherwise = findLeft f (ListZipper ls l (x :. rs))
...
findRight f (ListZipper ls x (r :. rs))
  | f r = IsZ $ ListZipper (x :. ls) r rs
  | otherwise = findRight f (ListZipper (x :. ls) r rs)
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop (ListZipper Nil x rs) = let (last :. rest) = reverse (x :. rs)
                                     in ListZipper rest last Nil
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop (ListZipper ls x Nil) = let (last :. rest) = reverse (x :. ls)
                                      in ListZipper Nil last rest
...
moveLeftN ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
moveLeftN n lz@(ListZipper ls x rs) 
  | n == 0 = IsZ lz
  | n > 0 = case ls of
      Nil -> IsNotZ
      (head :. tail) -> moveLeftN (n - 1) (ListZipper tail head (x :. rs))
  | otherwise = case rs of
      Nil -> IsNotZ
      (head :. tail) -> moveLeftN (n + 1) (ListZipper (x :. ls) head tail)
...
-- prop> rights (end lz) == Nil
  ...
  ListZipper a
  -> ListZipper a
end (ListZipper l x r) = let (head :. tail) = reverse (x :. r)
                         in ListZipper (tail ++ l) head Nil
