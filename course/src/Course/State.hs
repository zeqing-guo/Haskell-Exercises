...
instance Functor (State s) where
  (<$>) ::
    ...
    -> State s a
    -> State s b
  f <$> (State fa) = State (\s -> let (a, ss) = fa s
                                  in (f a, ss))
...
instance Applicative (State s) where
  ...
  (<*>) ::
    ...
    -> State s a
    -> State s b
  (State f) <*> (State fa) = State $ \s -> let (fab, s1) = f s
                                               (a, s2) = fa s1
                                           in (fab a, s2)
...
findM f (l :. ls) = f l >>= (\p -> if p then return $ Full l else findM f ls)
...
listWithState ::
  (Ord a, Monad f) =>
  ((a -> State (S.Set a) Bool) -> List a -> State (S.Set a) (f a))
  -> (a -> S.Set a -> Bool)
  -> List a
  -> f a
listWithState f p ls = eval (f predicateWithState ls) S.empty
  where
    predicateWithState ele =
      do s <- get
         put (S.insert ele s)
         return $ p ele s
-- A more beautiful version: listWithState f m x = eval (f (State . lift2 (lift2 (,)) m S.insert) x) S.empty
...
isHappy ::
  Integer
  -> Bool
isHappy n = contains 1 $ firstRepeat (produce (toInteger . sum . map (join (*) . digitToInt) . show') n)
-- Compute a square: join (*)
