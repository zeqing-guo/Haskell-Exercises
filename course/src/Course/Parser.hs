...
mapParser ::
  ...
  -> Parser a
  -> Parser b
mapParser f a = P $ \i -> case parse a i of
  Result i1 a1 -> Result i1 (f a1)
  ErrorResult ep -> ErrorResult ep
...
bindParser ::
  ...
  -> Parser a
  -> Parser b
bindParser f a = P $ \i -> case parse a i of
  Result i1 a1 -> parse (f a1) i1
  ErrorResult ep -> ErrorResult ep
...
list1 ::
  Parser a
  -> Parser (List a)
list1 pa = bindParser (\a -> (a :.) <$> list pa) pa
...
satisfy ::
  ...
  -> Parser Char
satisfy p = 
  flbindParser character (\c -> if p c
                                   then return c
...
natural ::
  Parser Int
natural =
  flbindParser (list1 digit) (\xs -> case read xs of
...
sequenceParser ::
  ...
  -> Parser (List a)
sequenceParser =
  foldRight (\a xs -> (:.) <$> a <*> xs) (valueParser Nil)
...
surnameParser = flbindParser upper
  (\c -> flbindParser (thisMany 5 lower)
         (\cs -> flbindParser (list lower)
                 (\css -> valueParser (c :. cs ++ css))))
...
phoneParser =
  ...
  (\d -> flbindParser phoneBodyParser
         (\ds -> flbindParser (is '#') (\_ -> valueParser (d :. ds))))
